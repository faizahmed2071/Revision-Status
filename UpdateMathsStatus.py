...
def UpdateMaths():
 newWindow = Tk()
 newWindow.title("Update Maths Status") 
 ...
 def update():
      ...
      try:
       ...
       mycursor.execute("SELECT * FROM maths where CHAP_NO = '{0}'" .format(chapter_number))
       ...
       if len(result) == 0:
        ...
        if result == 'yes':
         mycursor = mydb.cursor()
         mycursor.execute("update maths set STATUS= 'COMPLETED' where CHAP_NO= '{0}'" .format(chapter_number))
         mydb.commit()
