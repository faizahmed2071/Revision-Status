...
def UpdateChemistry():
 newWindow = Tk()
 newWindow.title("Update Chemistry Status") 
 ...
 def update():
      ...
      try:
       ...
       mycursor.execute("SELECT * FROM chemistry where CHAP_NO = '{0}'" .format(chapter_number))
       ...
       if len(result) == 0:
        ...
        if result == 'yes':
         mycursor = mydb.cursor()
         mycursor.execute("update chemistry set STATUS= 'COMPLETED' where CHAP_NO= '{0}'" .format(chapter_number))
         mydb.commit()
